...
class response_switch(Component):
    ...
    def start(self):
        ...
        if re.search("pcode|p-code", question, re.IGNORECASE):
            action = "p_code"
        elif re.search("(place|put|paste|insert|where does|add).+(tag|code|pixel)", question, re.IGNORECASE):
            action = "tag_placement"
        elif re.search("(where|find|what).+(tag|code|pixel)", question, re.IGNORECASE):
            action = "tag"
